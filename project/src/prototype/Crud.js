...
const Crud = () => {
                        ...
                        </tr>
                {
                    lists.map((current) => (
                        ...
                        <tr>
                            ...
                            <td>{current.damage}</td>
                            <td>{current.armor}</td>
                            <td>{current.bot_class}</td>
                            <td>{current.catchphrase}</td>
                            <td>{current.avatar_url}</td>
    ...
    function handleSubmit(event){
        ...
        const damage = event.target.elements.damage.value
        const armor = event.target.elements.armor.value
        const bot_class = event.target.elements.bot_class.value
        const catchphrase = event.target.elements.catchphrase.value
        const avatar_url = event.target.elements.avatar_url.value
        const created_at = event.target.elements.created_at.value
        const updated_at = event.target.elements.updated_at.value

        const newList = lists.map((li) => (
            li.id === updateState ? { ...li, name:name, health:health, damage:damage, armor:armor, bot_class:bot_class, catchphrase:catchphrase, avatar_url:avatar_url, created_at:created_at, updated_at:updated_at} : li
        ))
...
const EditList = ({current, lists, setList}) =>{
    ...
    const handInputhealth= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, health:value} : li
        ))

        setList(newList)
    }

    const handInputdamage= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, damage:value} : li
        ))

        setList(newList)
    }

    const handInputarmor= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, armor:value} : li
        ))

        setList(newList)
    }

    const handInputbot_class= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, bot_class:value} : li
        ))

        setList(newList)
    }

    const handInputcatchphrase= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, catchphrase:value} : li
        ))

        setList(newList)
    }

    const handInputavatar_url= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, avatar_url:value} : li
        ))

        setList(newList)
    }

    const handInputcreated_at= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, created_at:value} : li
        ))

        setList(newList)
    }

    const handInputupdated_at= (event) =>{
        const value = event.target.value;
        const newList = lists.map((li) => (
            li.id === current.id ? { ...li, updated_at:value} : li
        ))
    ...
    return(
        <tr>
            ...
            <td><input type='text' onChange={handInputhealth} name='health' value={current.health} lists={lists} setList={setList}/></td>
            <td><input type='text' onChange={handInputdamage} name='damage' value={current.damage} lists={lists} setList={setList}/></td>
            <td><input type='text' onChange={handInputarmor} name='armor' value={current.armor} lists={lists} setList={setList} /></td>
            <td><input type='text' onChange={handInputbot_class} name='bot_class' value={current.bot_class} lists={lists} setList={setList}/></td>
            <td><input type='text' onChange={handInputcatchphrase} name='catchphrase' value={current.catchphrase} lists={lists} setList={setList}/></td>
            <td><input type='text' onChange={handInputavatar_url} name='avatar_url' value={current.avatar_url} lists={lists} setList={setList} /></td>
            <td><input type='text' onChange={handInputcreated_at} name='created_at' value={current.created_at} lists={lists} setList={setList}/></td>
            <td><input type='text' onChange={handInputupdated_at} name='updated_at' value={current.updated_at} lists={lists} setList={setList}/></td>
...
const AddList = ({setList}) => {
    ...
    const bot_classRef = useRef()
    const catchphraseRef = useRef()
    const avatar_urlRef = useRef()
...
armorRef.current.value = ""
bot_class.current.value = ""
catchphrase.current.value = ""
avatar_url.current.value = ""
created_at.current.value = ""
updated_at.current.value = ""

    }
    return(
        <form className="addForm" onSubmit={handleSubmit}>
            <input type="text" name="name" placeholder="Name" ref={nameRef}/>
            <input type="text" name="health" placeholder="Health" ref={healthRef}/>
            <input type="text" name="damage" placeholder="Damage" ref={damageRef}/>
            <input type="text" name="armor" placeholder="Armor" ref={armorRef}/>
            <input type="text" name="bot_class" placeholder="Bot class" ref={bot_classRef}/>
            <input type="text" name="catchphrase" placeholder="Catchphrase" ref={catchphraseRef}/>
            <input type="text" name="avatar_url" placeholder="Avatar url" ref={avatar_urlRef}/>
            <input type="text" name="created_at" placeholder="Created at" ref={created_atRef}/>
            <input type="text" name="updated_at" placeholder="Updated at" ref={updated_atRef}/>
